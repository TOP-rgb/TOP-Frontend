    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        {loadingAvail && (
          <span className="text-xs text-slate-400 dark:text-slate-500 flex items-center gap-1">
            <div className="w-3 h-3 border border-slate-300 dark:border-slate-600 border-t-transparent rounded-full animate-spin" />
            Checking…
          </span>
        )}
      </div>
      ...
      <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden text-xs">
        ...
        <div className="flex bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
          <div className="w-20 sm:w-28 shrink-0 px-2 py-1.5 font-medium text-slate-500 dark:text-slate-400">Attendee</div>
          {slots.map((s, i) => (
            <div
              key={i}
              className="flex-1 text-center py-1.5 border-l border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 truncate px-0.5"
              style={{ minWidth: 0 }}
            >
        ...
        {attendeeIds.map((uid) => {
          ...
          return (
            <div key={uid} className="flex border-b border-slate-100 dark:border-slate-700/50 last:border-0">
              <div
                className="w-20 sm:w-28 shrink-0 px-2 py-1.5 font-medium text-slate-700 dark:text-slate-300 truncate"
              ...
              {slots.map((s, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex-1 border-l border-white dark:border-slate-800 py-1.5 ${bg}`}
                    title={title}
                    style={{ minWidth: 0 }}
                  />
                )
        ...
        {/* Legend */}
        <div className="flex items-center gap-3 px-2 py-1.5 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700">
          <span className="flex items-center gap-1 text-slate-500 dark:text-slate-400">
            <span className="w-3 h-3 rounded-sm bg-green-100 dark:bg-green-900/30 border border-slate-200 dark:border-slate-700" /> Free
          </span>
          <span className="flex items-center gap-1 text-slate-500 dark:text-slate-400">
            <span className="w-3 h-3 rounded-sm bg-red-200 dark:bg-red-900/40 border border-slate-200 dark:border-slate-700" /> Busy
          </span>
          <span className="flex items-center gap-1 text-slate-500 dark:text-slate-400">
            <span className="w-3 h-3 rounded-sm bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700" /> Unknown
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 shrink-0">
          <h3 className="font-semibold text-slate-900 dark:text-slate-100">
            {existingId ? 'Edit Event' : 'New Event'}
          </h3>
          <div className="flex items-center gap-3">
            {syncBadges.length > 0 && (
              <span className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-full px-2.5 py-0.5">
        ...
        <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600"
            />
          </div>

          {/* Event type */}
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Type</label>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-all ${
                      form.eventType === t
                        ? 'text-white'
                        : 'text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'
                    }`}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <label htmlFor="allDay" className="text-sm text-slate-700 dark:text-slate-300">All day</label>
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">
              <span className="flex items-center gap-1">
                ...
                {totalAttendees > 0 && (
                  <span className="ml-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded-full text-xs px-1.5 py-0.5 font-medium">
                    {totalAttendees}
                  </span>
                )}
              </span>
            ...
            <div className="relative mb-2">
              <div className="flex items-center gap-2 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500 dark:bg-slate-700">
                ...
                <input
                  ...
                  className="flex-1 text-sm outline-none bg-transparent dark:text-slate-100 dark:placeholder-slate-400"
                />
              </div>
              {showUserDropdown && userSearch && filteredUsers.length > 0 && (
                <div className="absolute z-10 top-full mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-44 overflow-y-auto">
                  {filteredUsers.slice(0, 8).map((u) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-slate-800 dark:text-slate-200 truncate">{u.firstName} {u.lastName}</div>
                        <div className="text-xs text-slate-400 dark:text-slate-500 truncate">{u.email}</div>
                      </div>
                      <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0">{u.role}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* External attendees */}
            <div className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 space-y-2 bg-slate-50 dark:bg-slate-900/50">
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1">
                <Mail className="w-3 h-3" /> Add external attendee
                <span className="text-slate-400 dark:text-slate-500 font-normal">(outside your org — team members use the search above)</span>
              </p>
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="flex-1 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600"
                />
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-t border-slate-100 dark:border-slate-700/50 shrink-0">
          <div>
            {existingId && onDelete && (
              <button
                type="button"
                ...
                disabled={deleting}
                className="flex items-center gap-1.5 text-sm text-red-600 hover:text-red-700 px-3 py-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50"
              >
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg"
            >
              Cancel
            </button>
            <button
...
function EventDetailModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md flex flex-col overflow-hidden"
      ...
      >
        ...
        <div className="px-5 py-4 space-y-3 overflow-y-auto max-h-[60vh]">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-slate-700 dark:text-slate-300">
              {event.allDay
                ...
                : <>{fmt(event.startAt)}<span className="text-slate-400 dark:text-slate-500 mx-1">→</span>{fmt(event.endAt)}</>
              }
            </div>
          </div>
            ...
            if (isExternal) {
              ...
              return (
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-slate-700 dark:text-slate-300">
                    <span className="text-slate-400 dark:text-slate-500 text-xs">Organized by </span>
          ...
          {event.location && (
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-slate-700 dark:text-slate-300">{event.location}</div>
            </div>
          )}
          ...
          {event.description && (
            <div className="flex items-start gap-3">
              <AlignLeft className="w-4 h-4 text-slate-400 dark:text-slate-500 mt-0.5 shrink-0" />
              <div className="text-sm text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{event.description}</div>
            </div>
          )}
                ...
                <div className="space-y-1.5">
                  {allAttendees.map((a, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800/60 flex items-center justify-center text-xs font-semibold text-slate-500 dark:text-slate-400 shrink-0">
                        {(a.name || a.email || '?')[0].toUpperCase()}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-slate-700 dark:text-slate-300 leading-none">{a.name}</div>
        ...
        {/* Footer actions */}
        <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-700/50 flex items-center gap-2 flex-wrap">
          {/* Join meeting button */}
          {event.meetingUrl && (
            <a
          ...
          {isOwner && (
            <button
              onClick={onEdit}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50"
            >
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {allowGoogle && (
        googleInt ? (
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-full px-3 py-1"
          >
      ...
      {(googleInt || msInt) && (
        <button
          onClick={onSync}
          disabled={syncing}
          className="flex items-center gap-1.5 text-xs border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-full px-3 py-1 disabled:opacity-50"
        >
...
export function Calendar() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-white dark:bg-slate-800">
      {/* Top bar */}
      <div className="flex flex-col gap-2 sm:gap-3 px-4 sm:px-5 py-3 border-b border-slate-100 dark:border-slate-700/50">
        {/* Row 1: nav + title + [desktop: view selector] + new event */}
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <button
              ...
              className="p-2 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-400"
              title="Previous"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-400"
            >
              Today
            </button>
            <button
          ...
          {/* Current title */}
          <span className="text-base font-semibold text-slate-900 dark:text-slate-100 flex-1 min-w-0 truncate">{currentTitle}</span>

          {/* View selector — desktop only */}
          <div className="hidden sm:flex border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden text-sm">
            {(['dayGridMonth', 'timeGridWeek', 'timeGridDay', 'listWeek'] as const).map((v) => (
              <button
                key={v}
                onClick={() => switchView(v)}
                className={`px-3 py-1.5 ${currentView === v ? 'bg-slate-100 dark:bg-slate-800/60 font-medium dark:text-slate-100' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-400'}`}
              >
                {viewLabel(v)}
              </button>
            ))}
          ...
          {/* New event — icon only on mobile, full label on desktop */}
          <button
            onClick={handleNewEvent}
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 shrink-0"
          >
        ...
        {/* Row 2: view selector — mobile only, full-width scrollable pills */}
        <div className="flex sm:hidden overflow-x-auto hide-scrollbar border border-slate-200 dark:border-slate-700 rounded-lg">
          {(['dayGridMonth', 'timeGridWeek', 'timeGridDay', 'listWeek'] as const).map((v) => (
            <button
              key={v}
              onClick={() => switchView(v)}
              className={`flex-1 px-3 py-1.5 text-sm whitespace-nowrap ${currentView === v ? 'bg-slate-100 dark:bg-slate-800/60 font-medium text-slate-900 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
            >
              {viewLabel(v)}
            </button>
          ))}
